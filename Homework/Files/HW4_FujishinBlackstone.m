...
% part b
% create an interpolation of the function using the polyfit function and 
...
% calculate the difference between the polynomial and the exact function
% and plot the error.
err = p - y;
n1 = norm(err,1);
n2 = norm(err,2);
...
% part c
% The chebyshev and legendre points produce very small errors, legendre
...
% and the remez function to find the best interpolation with 14 degrees.
f = chebfun('exp(2*x)');
[r,errr] = remez(f,14);
...
%part c
x1=[x(end-1);x;x(2)]; % here we create new data points by correcting and adding the points after the end and the points before.
y1=[y(end-1);y;y(2)];
t1=1:length(x1); % again to parameterize
tt1=linspace(t1(1),t1(end),N+1);
% Now we run our new data through to see if it produces a better looking interpolation:
